...
- **READ:** Source code in `src/holus/`, configuration files in `config/`, `.env.example` (template only - never `.env`), `config/guardrails.yaml`, git history (last 30 commits via `git log --diff-filter=M --name-only`), `agentic/agents/AGENTS.yaml`, MCP configuration files
