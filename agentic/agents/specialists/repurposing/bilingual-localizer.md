...
- **READ:** Original English LinkedIn post (primary source), `config/brand.yaml` (voice section - tone, anti-patterns, language rules: `spanish_register: "tu"`, `spanish_note: "Translate concepts, not words. Technical terms stay in English when natural"`), `agentic/memory/knowledge/current/platforms.md` (Facebook adaptation rules: warmer tone, full LinkedIn length acceptable, bilingual CTA format)
