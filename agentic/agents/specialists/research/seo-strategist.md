...
- **READ:** `config/brand.yaml` (content_pillars, target_client, positioning), `agentic/memory/knowledge/current/niche-research-queries.md` (trending topics as keyword signals), existing blog posts via portfolio MCP or file read (deduplication), web search results for keyword volume proxies
