...
- **READ:** `config/brand.yaml` (competitor_accounts — handles if filled in, search_categories as fallback), `.self-improvement/knowledge/current/niche-research-queries.md` (competitor_content query category), `.self-improvement/MEMORY.md` (prior competitive observations), `config/brand.yaml` (positioning, differentiation — our baseline)
