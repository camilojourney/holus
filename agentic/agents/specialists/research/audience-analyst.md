...
- **READ:** Social-media MCP `get_analytics(days=28)` and `get_top_posts(limit=20, metric="engagement_rate")`, `docs/target.md` (current persona definition), `config/brand.yaml` (target_client, content_pillars), `.self-improvement/MEMORY.md` (prior audience observations)
- **WRITE:** `.self-improvement/reports/marketing/audience-YYYY-MM-DD.md` (segment analysis report), `.self-improvement/MEMORY.md` (append audience insights section if findings are significant)
