...
- **READ:** published post content (to understand what was claimed and what was asked), incoming comments (text + commenter profile summary if available), commenter context (company size, title, engagement history), `agentic/memory/knowledge/current/platforms.md` (LinkedIn engagement tactics: reply to every comment in first hour, ask follow-up questions, DM trigger signals)
