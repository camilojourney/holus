...
- **READ:** `config/brand.yaml` (voice.closers, voice.tone, cta.primary, cta.secondary, positioning.what_i_am_not), `agentic/memory/knowledge/current/voice-profile.md` (closing patterns table with frequency data), the approved hook + body text (provided as input to understand the post's emotional arc and ending point)
