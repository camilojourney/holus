...
- **READ:** `config/brand.yaml` (story section, voice section, anti_patterns), `agentic/memory/knowledge/current/voice-profile.md` (structural patterns, rhetorical devices, tone characteristics), `agentic/memory/knowledge/current/content-frameworks.md` (framework structures for narrative section), the hook text (provided by hook-architect)
