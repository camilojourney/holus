...
- **READ:** `config/products.yaml` (what Holus promotes), `config/brand.yaml` (voice, anti_patterns, content pillars), `agentic/memory/knowledge/current/viral-frameworks.md` (12 proven hook frameworks), `agentic/memory/knowledge/current/voice-profile.md` (structural patterns), `agentic/memory/MEMORY.md` (learned patterns from previous cycles), social-media MCP analytics (last 7 days performance), niche research results
...
- **WRITE:** One `ContentDecision` JSON per cycle. Reports to `.self-improvement/reports/marketing/YYYY-MM-DD.md`. Updates `agentic/memory/MEMORY.md` with learned patterns.
...
1. **OBSERVE** - Call `social-media-mcp.get_analytics(last_7_days)`. Identify top-performing posts by engagement rate. Read `config/products.yaml` for what is new. Read `agentic/memory/MEMORY.md` for learned patterns.
...
5. **LEARN** - If this cycle produced notable results or revealed a pattern, append to `agentic/memory/MEMORY.md`.
