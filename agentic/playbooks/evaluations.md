...
2. **Domain-Expert Judges** (`agents/evaluators/`) — LLM-based evaluation after generation
   ...
   - Each judge is a domain expert with a category-specific rubric defined in `agents/AGENTS.yaml`
...
Each specialist in `agents/AGENTS.yaml` declares which evaluator scores it via `evaluated_by`. The brand-safety-judge runs as a cross-cutting gate on all content flagged with `gate: true`.
...
Each evaluator has weighted rubric dimensions defined in `agents/AGENTS.yaml`:
...
2. Add entry to `agents/AGENTS.yaml` under the evaluators section with `type: evaluator`, `model_tier: classification`, and `rubric` list
3. Update specialist entries in `agents/AGENTS.yaml` — set `evaluated_by: {name}` on the relevant specialists
