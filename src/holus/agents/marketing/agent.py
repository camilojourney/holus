    ...
    async def observe(self, state: MarketingState) -> dict[str, Any]:
        ...
        try:
            researcher = NicheResearcher(
                self.claude, self.config.anthropic_api_key, self.agent_name
            )
            niche_research = await researcher.research()
        except Exception:
    ...
    async def reason(self, state: MarketingState) -> dict[str, Any]:
        ...
        if self.config.anthropic_api_key:
            strategy_prompt = OPUS_STRATEGY_PROMPT.format(
                ...
                niche_research=NicheResearcher.format_niche_research(
                    state.get("niche_research", {})
                ),
