...
- **READ:** `.self-improvement/knowledge/current/niche-research-queries.md` (query bank + rotation state), `config/brand.yaml` (competitor_accounts, content pillars, target client pain points), `data/.niche-research-state.json` (last-run timestamps per category)
